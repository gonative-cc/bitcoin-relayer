...
import (
	...
	"github.com/gonative-cc/bitcoin-relayer/types"
)
...
func (r *Relayer) checkConsistency() (*consistencyCheckInfo, error) {

	tipRes, err := r.nativeClient.BTCHeaderChainTip()
	if err != nil {
		return nil, err
	}

	// Find the base height of BBN header chain
	baseRes, err := r.nativeClient.BTCBaseHeader()
	if err != nil {
		return nil, err
	}
...
func (r *Relayer) bootstrap(skipBlockSubscription bool) error {
	...
	signer := r.nativeClient.MustGetAddr()
...
func (r *Relayer) reporterQuitCtx() (context.Context, func()) {
	quit := r.quitChan()
...
func (r *Relayer) checkHeaderConsistency(consistencyCheckHeight uint64) error {
	var err error
	...
	res, err := r.nativeClient.ContainsBTCBlock(&consistencyCheckHash) // TODO: this API has error. Find out why
	if err != nil {
		return err
	}
	if !res.Contains {
